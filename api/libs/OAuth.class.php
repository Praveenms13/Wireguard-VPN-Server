...
class OAuth  //OAuth can be used to generate new refresh tokens
{
    ...
    public function authenticate()
    {
        if ($this->access_token) {
            $query = "SELECT * FROM `API_Sessions` WHERE `access_token` = '$this->access_token'";
    ...
    public function newSession($validfor = 3600, $reference_token = "Auth_Grant")
    {
        ...
        $query = "INSERT INTO `API_Sessions` (`username`, `access_token`, `refresh_token`, `reference_token`, `valid_for`)
    ...
    public function refreshSession()
    {
        if ($this->refresh_token and (!$this->stringStart($this->refresh_token, "NR."))) {
            $query = "SELECT * FROM `API_Sessions` WHERE `refresh_token` = '$this->refresh_token'";
            $result = $this->db->query($query);
            if ($result) {
